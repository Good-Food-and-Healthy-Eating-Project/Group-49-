...
class ClientsTableTest{
    ...
    fun should_insert_client_success(){
        transaction{
            ...
            val client = Clients
            .selectAll()
            .where{Clients.client_id eq test_id}
            .single()
    ...
    fun should_update_height_and_weight(){
        transaction{
            ...
            Clients.update({Clients.client_id eq test_id}){
                it[height_cm] = 160
                it[weight_kg] = 60
            }
    ...
    fun should_delet_client_success(){
        transaction{
            ...
            Clients.deleteWhere{Clients.client_id eq test_id}
            val deleteClient = Clients
            .selectAll()
            .where{Clients.client_id eq test_id}
            .singleOrNull()
    ...
    fun should_get_client_from_id(){
        transaction{
            ...
            .where{Clients.client_id eq test_id}
            .single()
            assertEquals(test_id,client[Clients.client_id])
            assertEquals(180,client[Clients.height_cm])
            assertEquals(80,client[Clients.weight_kg])
        }
    }
