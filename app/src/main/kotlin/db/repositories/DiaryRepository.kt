...
object DiaryRepository {
    ...
    fun findLogsByUserAndDateRange(userId: Int, start: Instant, end: Instant): List<FoodLogRecord> = transaction {
        FoodLogs
            .selectAll()
            .where {
    ...
    fun findAvailableDiaryWeeks(userId: Int): List<LocalDate> = transaction {
        FoodLogs
            .selectAll()
    ...
    fun findNutritionTotalsByLogIds(logIds: List<Int>): Map<Int, NutritionTotals> = transaction {
        if (logIds.isEmpty()) {
        ...
        (FoodLogItems innerJoin Foods)
            ...
            .mapValues { (_, rows) ->
                rows.fold(NutritionTotals(0.0, 0.0, 0.0, 0.0)) { totals, row ->
                    val quantityFactor = row[FoodLogItems.quantity_g].toDouble() / 100.0
                    NutritionTotals(
