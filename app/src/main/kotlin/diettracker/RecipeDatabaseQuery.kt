...
object RecipeDatabaseQuery {
    fun searchRecipes(
        query: String,
        favouriteIds: List<Int> = emptyList(),
        category: String = "",
    ): List<RecipeSummary> =
        transaction {
            Recipes
                ...
                .where {
                    val queryCondition =
                        if (query.isBlank()) {
                            ...
                            Recipes.recipe_name.lowerCase() like "%${query.lowercase()}%"
                        }
                    val categoryCondition =
                        if (category.isBlank()) {
                            Op.TRUE
                        } else {
    ...
    fun getRecipeById(recipeId: Int): RecipeDetails? =
        transaction {
            val recipe =
                Recipes
                    .selectAll()
            ...
            val ingredients =
                (RecipeIngredients innerJoin Foods)
                    .selectAll()
    ...
    fun searchByIngredient(ingredient: String): List<RecipeSummary> =
        transaction {
            val matchingFoodIds =
                Foods
                    .selectAll()
...
object RecipeFavouriteQuery {
    ...
    fun getFavourites(userId: Int): List<Int> =
        transaction {
            val results =
                UserFavouritedRecipes
                    .selectAll()
    ...
    fun getFavouriteRecipes(favouriteIds: List<Int>): List<RecipeSummary> =
        transaction {
            Recipes
                .selectAll()
...
object RecipeReviewQuery {
    ...
    fun getAverageRating(recipeId: Int): Double? =
        transaction {
            val reviews =
                RecipeReviews
                    .selectAll()
